...
- **デフォルト折りたたみ**: 子要素を持つリスト項目は、初期状態で折りたたまれます
- **クリックで展開**: 項目をクリックすると子要素が展開されます
- **展開状態の保持**: `sessionStorage` によりページ遷移後も展開状態が保持されます
- **JavaScript 依存**: ブラウザの JavaScript が有効である必要があります
- **アクセシビリティ**: HTML5 ネイティブ要素を使用するため、スクリーンリーダー等に対応
...
ページ遷移後にブラウザの戻るボタン等で戻った場合でも、展開状態が保持されます。

- **ストレージ**: `sessionStorage` を使用（ブラウザのタブを閉じるとクリアされます）
- **キー**: ページの URL パス（`location.pathname`）をベースに生成
- **識別**: 各 `<details>` 要素はインデックスと `<summary>` のテキスト内容で一意に識別
- **タイミング**: `<details>` の `toggle` イベントで即時保存、`DOMContentLoaded` で復元
