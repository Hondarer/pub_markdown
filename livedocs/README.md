...
選択したバリアントの言語と details を反映したタイトルを使用し、README またはタイトルがない場合はフォルダー名を表示します。

配信せずにサイトを一括生成する場合や、リンク切れの確認だけを行う場合は次を実行します。

```bash
make livedocs
```

`LIVEDOCS_STRICT=1` を付けると `mkdocs build --strict` になり、警告が 1 件でもあれば失敗します。  
...
serve だけを止めて `pages/livedocs/` を残す場合は次を実行します。

```bash
make stopdocs
```

停止の対象範囲と Windows でのツリー終了は [設計ドキュメント](../docs/livedocs-design.md) を参照してください。
...
```bash
python3 framework/docsfw/livedocs/bin/stage_livedocs.py --workspaceFolder="$PWD"
