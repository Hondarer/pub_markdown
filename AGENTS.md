...
- `bin/prepare_puppeteer_env.sh`、`bin/chrome-wrapper.sh`、`bin/mmdc-wrapper.sh` - ブラウザ依存処理の補助
...
- Linux と Windows でブラウザ起動経路が異なる。Puppeteer、Edge、Chromium 関連の変更では `pub_markdown_core.sh` とラッパースクリプトを同時に確認すること。
