...
- ブラウザー起動を変更する場合は `pub_markdown_core.sh` と `bin/prepare_puppeteer_env.sh`、`bin/chrome-wrapper.sh`、`bin/mmdc-wrapper.sh` の Linux / Windows 分岐
...
テンプレート内のリテラル `$` は Pandoc 用のエスケープが必要です。  
