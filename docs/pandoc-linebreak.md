...
Pandoc のソース コード調査により、改行コード処理が **個別の Writer モジュールではなく、ファイル出力ハンドラーで実装されている** ことが判明しました。具体的な処理フローは以下の通りです。
...
1. **Windows のデフォルト動作** - システムのテキスト モード ファイル I/O が自動的に `\n` を `\r\n` に変換
...
- R による後処理がシステム デフォルトの改行コードに戻してしまう
...
3. **Pandoc デフォルト ファイルを使用** - チーム全体での一貫性のために検討
