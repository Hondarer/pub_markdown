...
# docx カスタム プロパティのサンプル

このサンプルは、Pandoc の YAML メタデータから docx のカスタム プロパティを出力する例である。
...
一方、標準プロパティに該当しないルート階層の文字列メタデータは、docx 内の `docProps/custom.xml` にカスタム プロパティとして出力される。

このサンプルの `document-id` は標準プロパティではないため、docx ではカスタム プロパティ `document-id` になる。
...
コマンド ラインでは、docx を ZIP として扱い、`docProps/custom.xml` を確認できる。
...
`DOCPROPERTY` フィールドで存在しないカスタム プロパティを参照すると、Word はエラー文字列を表示する。

```text
エラー! プロパティ名が不明です。
```

このため、以下のようなフィールドは、docx 内に `document-id` カスタム プロパティが存在しない場合にエラー表示になる。
...
文書内で `document-id` を参照する場合は、値が未確定でも docx 内に `document-id` カスタム プロパティを作成しておく。
...
Pandoc 3.9 では、以下のいずれの書き方でも空文字のカスタム プロパティとして `docProps/custom.xml` に出力されることを確認している。
...
`keywords` は Pandoc から Word の組み込みプロパティ `Keywords` に対応する。未設定の場合でも組み込みプロパティとして扱われるため、カスタム プロパティ名が存在しない場合のエラーは避けやすい。

ただし、`keywords` は本来検索用キーワードであり、文書 ID 専用の意味を持つプロパティではない。検索用キーワードも同時に使用する文書では、文書 ID とキーワードが混在する。

文書 ID としての意味を優先する場合は `document-id` カスタム プロパティを使用し、表示時の未設定エラー回避を優先する場合は組み込みプロパティの流用を検討する。
