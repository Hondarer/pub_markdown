...
- **初回表示 (open-level 指定なし)**: すべての項目が折りたたまれた状態 (既定)
- **初回表示 (open-level=N 指定あり)**: 第 N 階層まで展開された状態
- **ブラウザーの「戻る」/「進む」(操作後)**: リスト操作時に保存された開閉状態が復元されます。
- **ブラウザーの「戻る」/「進む」(未操作)**: 保存済み状態がないため、初回表示と同じ状態になります。
- **ページのリロード**: 初回表示と同じ状態に戻ります (open-level 指定があれば再適用されます)。
- **タブを閉じる**: 保存された状態は破棄されます (`sessionStorage` のため)。
