...
コード ブロックのキャプションは、ブロックの直後に空行を挟んで `CodeBlock:` 行を記述して指定します。  
[表の `Table:`](table-caption.md) と同じく、キャプションをフェンスの外に出す記法です。  
コード ブロックのキャプションはブロックの下に表示し、表のキャプションは表の上に表示します。
...
- キャプションは `CodeBlock:` に続けて記述します。前後の空白は除去されます。
