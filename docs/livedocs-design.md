...
`partials/header.html` へ、配色トグルより前で `partials/docsfw-header-meta.html` を include します。  
並びは「タイトル … 発行者、発行日時、配色トグル、Doxygen、Git、検索」になり、  
静的発行のナビバー「タイトル … 発行者、発行日時、Doxygen、Git」と同じ順序になります。  
配色トグルと検索は静的発行に対応する要素を持たないため、両者の間に入ります。
...
Material は 1600px 以上で `html` の文字サイズを 137.5% へ上げますが、ヘッダーはスクリーンショット相当の表示より拡大する必要がないため、サイト名とページ名は 18px、検索入力欄は 16px に固定します。  
...
非表示の境は `76.25em` とし、アイコンの 767px より早く消します。  
発行日時は「曜日, 日 月 年 時刻 時差 短縮コミット ID」と長く、ページ タイトルの表示幅を圧迫するためです。  
`76.25em` は Material がヘッダーをドロワーへ切り替える境でもあります。
