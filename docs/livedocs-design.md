...
モード切替、Doxygen、Git の 24px アイコンの内側余白は 4px にします。  
...
Material 標準の SVG とロゴ、および Doxygen・Git のアイコンも、既存の最小値である 24px に固定します。  
...
「寸法と幅不足時の表示」で述べた `html` の font-size 拡大は 1600px (100em) 以上の 137.5% だけでなく、  
2000px (125em) 以上ではさらに 150% まで上がります。検索ボックスの幅は rem 指定のままのため、  
この 2 段目の拡大が直接効いて、100em や 125em を超えるとさらに幅が広がり続けます。

ヘッダーの他要素と同じく、既定の等倍 (`html` の 125%、20px 相当) での値を px に換算して固定します  
(`11.7rem` → `234px`、`23.4rem` → `468px`)。rem ではなく px にすることで、`html` の font-size が  
どこまで上がっても幅は変わりません。常時表示の入力欄と、検索を開いたときの候補パネルの両方を固定します。
...
幅と同じ理由で、ヘッダーの高さと余白も rem 指定のため画面幅により拡大します。  
文字とアイコンは px で固定済みのため、伸びるのは箱と余白だけになり、ヘッダーだけが間延びして見えます。  
実測では、ビューポート幅 1378px でヘッダーの高さが 49px、検索入力欄の高さが 36px なのに対し、  
2538px では 58px と 43px になり、入力欄の左右余白も 44px から 52.8px へ広がりました。
...
いずれも等倍で 48px の高さに収まります。この 48px を保つよう、各値を等倍での換算値へ px で固定します。

検索ボックス内の指定も同じ基準でそろえます。入力欄と入力候補の左右余白 `2.2rem` を 44px、  
虫めがねと消去ボタンの位置 `.3rem` と `.5rem` を 6px と 10px、  
候補パネルの上端 `1.9rem` を 38px にします。  
対象は入力欄として並ぶ 60em 以上だけです。60em 未満では検索も 40px 四方のボタンになり、  
`html` の font-size 拡大が始まる 1600px にも届かないためです。
