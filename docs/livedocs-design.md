...
これで、`set-meta.lua` が文書側のメタデータを上書きしない挙動がそのまま再現されます。

### 同じリポジトリ走査からの導出

発行者、発行日時、Git 単一ページ リンクは、いずれも同じ情報源から導けます。  
このため `bin/git_link.py` の `RepoInfo` を拡張し、リポジトリごとに 1 回だけ集める情報へ次を足しました。

- `git log --name-only` の 1 パスで、パスごとの最終コミット (`%H` `%h` `%cD`) とコミッター名の並び
...
結果として、生成 md では発行日時がファイルの更新時刻になり、発行者は表示されません。  
その挙動もそろえます。

### ヘッダーへの配置

`partials/header.html` のタイトル領域へ `partials/docsfw-header-meta.html` を include します。  
タイトル領域の既存 48px をタイトルと発行情報の 24px ごとの 2 行として使い、発行情報をタイトルの下へ置きます。  
タイトルには上側へ 2px の余白を設け、24px の行高にします。  
発行情報は 24px 位置に置き、上側へ 2px の余白を設けます。  
この配置はヘッダーの高さを増やさず、配色トグル、Doxygen、Git、検索の横幅も変えません。

言語による出し分けはありません。発行者はコミッター名、発行日時は RFC2822 で、どちらも言語に依存しないためです。

### 寸法と幅不足時の表示

発行者と発行日時の文字サイズは 14px に固定します。  
静的発行の `.docsfw-header-meta` と `.navbar-text` も同じ 14px です。  
`html` の font-size は 125% に固定し、サイト名とページ名は 18px、検索入力欄とサジェスチョンは 16px にします。  
検索入力欄とサジェスチョンには同じフォントと通常の文字間隔を指定し、入力中の文字と候補の字送りをそろえます。  
このため、ヘッダー内の全テキストは画面幅により拡大しません。  
左端のロゴとメニューは 24px、右上の操作アイコン (配色トグル、Doxygen、Git、検索) は 20px に固定します。  
ボタンの操作領域は変えないため、アイコンだけが画面幅により拡大しません。  
色は本文より一段淡い副次情報として `--md-primary-bg-color--light` を使います。  
この変数は `docsfw-pandoc-style.css` が `default` と `slate` の双方で定義しています。
...
画面幅で候補パネルの大きさを変えない方針のためです。

### ヘッダーの高さと余白

幅と同じ理由で、ヘッダーの高さと余白も 1rem = 20px の換算値を px にします。  
...
PlantUML と Mermaid へ、ソース／図の切り替え、SVG ダウンロード、コピーのボタンを右上に重ねます。  
...
このためライト テーマで描画した `<svg>` を `XMLSerializer` で直列化し、`Blob` として保存します。  
図を表示しているときのコピーは、同じ SVG を Canvas で透明背景の PNG に変換します。  
ソースを表示しているときは、利用者が記述した元ソースをテキストとしてコピーします。  
画像ファイルとして配置した SVG は、従来どおりダウンロードだけを提供します。
...
位置の基準はコード本文の可視領域であり、横スクロールの内容幅の右端ではありません。  
...
開閉処理が `pre` を内容幅まで伸ばすため、`.md-code__nav` は `.code-expander-scroll` の外へ移し、静的発行と同じく可視領域の右上へ固定します。
...
コード本文の内側余白は 11px 16px です。  
動的発行では Material が余白を `code` に持たせるため、先頭 5 行の `code` は下側、残りの `code` は上側の余白を 0 にし、5 行目と 6 行目の境界で余白が重ならないようにします。  
開閉バーの下端と「さらに N 行」の上端には `--docsfw-soft-border` を使い、コード ブロックの外枠より目立たない区切り線にします。

### コード ブロックの寸法

コード ブロックは、静的発行と動的発行の両方でコード本文を 14px、行送りを 19px、内側余白を上下 11px・左右 16px、ブロックの上下マージンを 15px とします。  
構文ハイライト付きとハイライトなしのどちらにも同じ値を使用します。

開閉バーは、ボタンの行送りを 22px、上下の余白を 4px、下端の区切り線を 1px とし、全体の高さを 31px にします。  
行送りを相対値にするとバーが小数の高さになり、配置座標によって描画時のピクセル数が変わるため、px の固定値を使用します。

静的発行では `pre > code` を `display: block` とします。  
`code` がインラインのままだと、親 `pre` の 16px フォントが作る strut に行ボックスが支配され、`code` の `line-height: 19px` を指定しても実際の行送りが 20px になるためです。  
動的発行では Material が同じ要素をブロック化しています。
...
PlantUML ソース内に `caption` がある場合は、ステージング時に `figure` と `figcaption` を生成します。  
...
ルートの `.nav.yml` には、本文の `\toc` と同じ並びになる `sort:` も設定します。  
ファイルとフォルダーを混在させ、ソース名 (`*.md` / フォルダー名) の大文字小文字を無視したアルファベット順です。  
`mkdocs-awesome-nav` の既定 (ファイルの後にフォルダー、自然順) は使いません。  
子ディレクトリの `.nav.yml` はこの `sort:` を継承します。  
...
Bootstrap のナビバー構造は使用せず、Material と同じ 48px のヘッダー本体と 12px の本文背景帯で構成します。  
ヘッダーは単色 `#F7F7F7` とし、幅の制限を受けない背景帯の上端へ 1px の境界線 (`#D4D4D4`) を引きます。
...
`html { font-size: 125% }` (1rem = 20px) を基準とし、広い画面でも 125% に固定します。

1rem = 20px として px へ換算した固定値を両側へ置きます。  
...
折りたたみボタンも、現在ページ・祖先・ホバー・フォーカスでは同じナビ色にします。  
...
本文の先頭見出しは、すべての画面幅でヘッダー下端から 28px の位置にそろえます。  
767px 以下の本文は、Pandoc HTML と MkDocs の両方で左右に 20px の余白を取ります。  
768px から 929px の本文は左右に 30px の余白を取り、930px で最大幅の 870px に達します。
...
本文の上端は、1 列表示でも 3 列表示と同じくヘッダー直後にそろえます。  
Material が本文側に持つ上マージンと上パディングは使用せず、ヘッダー自身が持つ 12px の背景帯だけを先頭余白にします。  
...
そのため約 1220px 未満では、表示中の板の一覧へ入れます。  
どの板を表示するかは、各階層のチェック ボックス (`.md-nav__toggle`) が checked かどうかで決まります。  
根の板から checked の子板をたどり、行き着いた板が表示中の板です。  
現在ページのリンクが属する一覧は、表示中の板の一覧とは限りません。  
現在ページ自身が節の索引ページ (`navigation.indexes`) のときは、その節自身の板が開いた状態で始まり、リンクは 1 つ上の一覧にあります。  
...
Material の `.md-nav--primary .md-nav__link` の `padding: 0.6rem 0.8rem` がタップしやすい高さを持ちます。  
Material は 60em 以上になるとページ内目次の各 `.md-nav__list` へ 12px の左余白を加えるため、960px から先頭階層の文字が右へずれます。  
ドロワー内のページ内目次ではリストの左余白を 0 にし、リンクの左余白を先頭階層 16px、子階層 32px として、Pandoc HTML と同じ位置へ固定します。  
1220px から 1399px の連続一覧では、外側の文書一覧が持つ 12px にページ内目次の 12px が加わるため、先頭階層は両発行とも 24px です。子階層だけを 16px 刻みに直し、40px でそろえます。  
項目ごとの区切り線も残し、ファイル一覧からの続きとして並べます。  
板見出しのフォルダー名はホバーしても下線を引きません。  
MkDocs の板見出しでは、操作できない余白とフォルダー名を通常カーソルにし、上位階層へ戻るアイコンだけをポインター カーソルにします。  
...
1220px から 1399px の連続一覧では、`.docsfw-combined-toc` の外側に 12px の上余白を置きます。  
区切り線の上下を同じ 12px にそろえ、最後の文書行と「目次」の間で区切り線が偏って見えないようにします。  
約 1220px 未満の階層ごとの板では上余白を置かず、区切り線を直前の文書行へ接して表示します。  
区切り線の下には 12px の内側余白を残し、「目次」の見出しと配下のリンクは狭幅の文書行と同じ左右 16px、高さ 45px にします。  
最狭ドロワーではリンクの上余白を 0 にし、1px の区切り線をリンクの外側のリスト項目へ置いて、MkDocs と同じ 46px 間隔で並べます。  
最狭ドロワーでは「目次」の見出しと先頭の項目の間にも同じ区切り線を置き、ほかの行と同じ間隔にそろえます。  
Pandoc は先頭のリスト項目に対する打ち消しを置きません。  
MkDocs は Material の `.md-nav--primary .md-nav__title ~ .md-nav__list > :first-child` の打ち消しを戻します。  
Material は見出しの内側の影が区切り線を兼ねる前提で先頭項目の線を消しますが、目次の見出しはこの影を消しているため、戻さないとここだけ線が無くなります。
...
この幅では内容下余白も持たせず、最後の行を Pandoc HTML の板と同じく画面下端へ接して終わらせます。  
Material は一覧の `padding-bottom: 0.4rem` を `.md-nav` の `margin-bottom: -0.4rem` で打ち消しますが、スクロール範囲は負のマージンを含めません。  
...
Pandoc HTML と MkDocs の判定線は、どちらも上端から 88px の位置です。  
アンカーの着地位置は 84px ですが、ブラウザーがスクロール位置を整数に丸めると見出しの上端が 84px をわずかに超える場合があります。  
判定線を 4px 下へ置くことで小数座標の差を吸収し、クリックした見出しを現在項目として選びます。

ドロワーを開いたときの自動表示対象は、ドロワーの構成に合わせて切り替えます。  
階層ごとの板になる狭幅では、ページ内目次の現在見出しを優先して表示します。  
現在見出しがない文書上端では、現在の文書リンクを表示します。  
1 本の連続一覧になる中幅では、現在の文書リンクを表示します。

現在の文書リンクとページ内目次の現在見出しは、DOM 上で同時に強調状態を持つ場合があります。  
ここでいう自動表示対象は、ドロワーのスクロール位置を合わせて利用者へ見せる項目です。  
`docsfw-responsive-nav.js` と Pandoc HTML の `openDrawer` は同じ優先順位で対象を選び、`scrollIntoView` で表示します。

ドロワーを開いたまま本文を移動した場合は、ページ内目次の現在見出しを表示範囲へ追従させます。  
MkDocs は Material の `toc.follow` を使用し、Pandoc HTML は `docsfw-nav.js` が表示中のスクロール領域を同じ位置へ動かします。  
項目がスクロール範囲の端に近く中央へ置けない場合でも、項目全体が表示範囲へ入ることを保証します。

板になる幅では、表示中の一覧に含まれる項目だけを自動表示の対象にします。  
表示していない板は `translateX` で右へ退避しているだけで、`.md-sidebar__scrollwrap` の `scrollWidth` は板の枚数だけ横に広がります。  
退避した板の項目へ `scrollIntoView` すると、ドロワー全体が横へスクロールして利用者が戻せなくなるためです。  
ドロワーを開くたびに `.md-sidebar__scrollwrap` の横位置も左端へ戻します。

### 静的発行側の対応

Pandoc HTML は Material の実行資産を読み込まず、`styles/html/docsfw-ui.css` と `docsfw-nav.js` で同じ寸法と操作を実装します。  
検索は 60em 以上で幅 234px の入力欄と幅 468px の候補パネルを表示し、それ未満ではヘッダー全幅の検索パネルへ切り替えます。  
