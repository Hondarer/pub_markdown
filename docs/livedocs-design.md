...
板になる幅では、現在の文書リンクが表示中の板にあるときだけ中央へ移します。  
表示していない板は `translateX` で右へ退避しているだけのため、`.md-sidebar__scrollwrap` の `scrollWidth` は板の枚数だけ横に広くなります。  
上位の板へ戻してからドロワーを開き直すと、現在の文書リンクは右へ退避した板の中にあります。  
そこへ `scrollIntoView` すると `.md-sidebar__scrollwrap` が横へスクロールし、ドロワーの中身が左へずれたままになります。  
`overflow-x: hidden` で横スクロール バーが出ないため、利用者には戻す手段がありません。  
Pandoc HTML の `openDrawer` も表示中の板の中の現在行だけを選んでおり、条件をそろえます。  
併せて、ドロワーを開くたびに `.md-sidebar__scrollwrap` の横位置を左端へ戻します。
