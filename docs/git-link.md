...
そこで doxyfw 側で、生成 md の先頭フロント マターに元ソース ファイルのパスをヒントとして埋め込みます。
...
docsfw は発行時に `$file` のフロント マターから `git-origin` を読み取り、`${workspaceFolder}/${git-origin}` が実体として存在すれば、md 自身ではなくこの元ソースに対して Git リンクを解決します。  
