...
| ナビゲーション ツリー | 全ページを階層ツリーで常時表示。現在ページをハイライトし祖先ディレクトリを展開 |
...
現在ページのノード配下に、そのページの見出し一覧 (pandoc `--toc` が生成する `$toc$`) を入れ子で表示します。  
全体の中での現在位置とページ内構造を 1 つのツリーで把握できます。

テンプレートは `$toc$` を `<div id="docsfw-page-toc" hidden>` で包んで初期非表示で出力します。  
`docsfw-nav.js` がツリー描画後に DOM 操作で現在ノードへ移設し、元のコンテナーを除去します。

- **フォールバック**: ナビゲーション ツリーに現在ページが見つからない場合 (`__DOCSFW_NAV__` 未生成時など) は  
  `hidden` を外してその場に表示します (従来の単独 TOC 表示)。
- **見出し無しページ**: `$toc$` が出力されないため、マージは行わず区切り線も除去して終了します。

### モバイル: 展開ボタンとオフ キャンバス ドロワー

スマートフォン幅 (≤767px) では全体ナビゲーション ツリーをオフ キャンバス ドロワーとして提供します。

- 左上に固定表示されるボタン (`›`) をタップするとサイドバーが左からスライド イン。  
  ドロワーが開くとボタンのアイコンが `‹` に変わり、ドロワーの右端へ移動します。
- ドロワー内は検索ボックス + 全体ナビゲーション ツリー + 現在ページの見出し (マージ済み) で構成されます。
- バック ドロップのタップ・Esc キー・ナビ リンクのクリックでドロワーを閉じます。
- 実装はサイドバー (`.span3`) を `position: fixed` のドロワーに変換する CSS と、  
