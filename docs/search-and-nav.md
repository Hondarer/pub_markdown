...
初回表示、通常のアンカー移動、戻る・進むによるスクロール位置の復元には、この位置補正を適用しません。

対象見出しが文書末尾に近く、ブラウザーが見出しを判定線まで移動できない場合も、アンカー移動直後はフラグメントが示す見出しを現在項目として表示します。  
その後に本文をスクロールすると、スクロール位置による選択へ戻ります。
...
- ページを下側へ進めるスクロール中にモバイル ブラウザーの操作領域が収納されると、表示領域が初期表示時より下側へ広がります。  
  スクロールする領域の高さは `dvh`、塗りつぶしだけの背景と覆いの高さは `lvh` を基準にします。  
  ドロワー本体を広げるとスクロール バーの領域も変わるため、疑似要素でドロワー下端から `100lvh` の背景だけを追加描画します。  
  疑似要素を切り取らないために、ドロワー本体の `overflow` は `visible` とし、内容の切り取りは `#docsfw-primary-sidebar > .well` が持ちます。  
  覆い (`#docsfw-nav-backdrop`) の高さも `lvh` を基準にし、操作領域の収納後に表示された本文を覆います。  
  MkDocs 側も同じ規則です。詳細は [動的発行基盤の該当節](livedocs-design.md) を参照してください。
- iOS では、meta viewport が既定の `viewport-fit=auto` のとき、表示領域が安全領域の分だけ小さくなります。  
  実機での計測では、画面の高さ 956px に対して表示領域が 840px、`viewport-fit=cover` を指定すると 874px でした。  
  差の 34px は `env(safe-area-inset-bottom)` と同じ値です。`vh`、`svh`、`lvh`、`dvh` はいずれもこの表示領域を基準にするため、  
  `lvh` では画面の下端まで届きません。テンプレートの meta viewport に `viewport-fit=cover` を指定し、表示領域を画面全体へ広げます。  
  広げた分は内容の領域にもなるため、端に接する箱へ `env(safe-area-inset-*)` の余白を足します。  
  本文とヘッダーは左右と下端、ドロワーと検索の一覧はスクロール範囲内の下端が対象です。  
  ドロワーの背景は画面の端まで敷き、内容だけを安全領域の内側へ入れます。
- スクロール中は、固定要素の下端が表示領域の下端へ追い付くまでに遅れがあります。実機での計測では最大 24px 程度でした。  
  塗りつぶしだけの覆いには `--docsfw-viewport-slack` の分の余裕を足し、その間も下端に本文が出ないようにします。  
  はみ出す分は画面の外に落ちます。スクロールする領域の高さは変更しません。
- 内容下余白は 1220px から 1399px で 12px、約 1220px 未満の板では 0 です。MkDocs もこの値にそろえます。
- バック ドロップ、Esc キー、ナビゲーション リンク、ページ内目次リンクでドロワーを閉じます。
- マウスまたはタッチで開閉した場合は、開閉後にフォーカス表示を残しません。キーボード操作 (click の `detail === 0`) ではフォーカスを維持します。  
  狭い画面では覆いがヘッダーの上に重なるため、指で閉じる経路は覆いのタップだけです。ここでもポインター操作かどうかを見ます。  
  Esc キーで閉じた場合は、メニュー ボタンへフォーカスを戻します。検索パネルの戻るボタンと覆いも同じ規則です。  
  iOS の WebKit は、キーボード操作前のプログラム フォーカスを `:focus-visible` とみなします。  
  リロード直後に閉じるとフォーカス枠が出るのはこのためで、`:focus:not(:focus-visible)` では消せません。フォーカス自体を当てないようにします。  
  MkDocs は開閉をフォーカスを受けない `<label>` と `display: none` のチェック ボックスで行うため、同じ規則を実装で持ちません。詳細は  
  [動的発行の設計](livedocs-design.md) を参照してください。
...
検索パネルとドロワーは同時に開きません。  
パネルの覆い (`.docsfw-search-backdrop`) の高さは、ドロワーの覆いと同じく `lvh` を基準にした `calc(100lvh - 48px)` です。  
結果一覧 (`#docsfw-search-results`) はスクロール コンテナーのため、高さは `dvh` 基準のままにして、末尾の行が操作領域の裏へ入らないようにします。  
入力欄のフォーカス枠は 1px です。
...
後処理が失敗した場合は、発行を失敗として終了します。  
失敗を無視すると、検索 UI を持つ HTML と欠落した `search-index.js` が混在し、検索だけが無反応になります。
