...
`docsfw-nav.js` がツリー描画後に DOM 操作で現在ノードへ移設し、元のコンテナを除去します。

- **フォールバック**: ナビツリーに現在ページが見つからない場合 (`__DOCSFW_NAV__` 未生成時など) は
  `hidden` を外してその場に表示します (従来の単独 TOC 表示)。
- **見出し無しページ**: `$toc$` が出力されないため、マージは行わず区切り線も除去して終了します。

### モバイル: ハンバーガーメニュー

スマートフォン幅 (≤767px) では全体ナビツリーをオフキャンバス ドロワーとして提供します。

- 左上に固定表示されるハンバーガーボタン (☰) をタップするとドロワーが左からスライドイン。
- ドロワー内は検索ボックス + 全体ナビツリー + 現在ページの見出し (マージ済み) で構成されます。
- バックドロップのタップ・Esc キー・ナビリンクのクリックでドロワーを閉じます。
