...
```javascript
puppeteer.launch(buildLaunchOptions())
```

- Linux/WSL では `chrome-wrapper.sh` を `executablePath` として直接指定します
- Windows ネイティブ Node.js では `chrome-wrapper.sh` を使わず、Puppeteer の起動設定に従います
...
DOCX 用 Mermaid 資産の競合回避は、[`mermaid.lua`](mermaid.md) の規則に従って発行スケジューラーが管理します。

同じ出力ディレクトリの Mermaid 文書だけを直列化し、共有ブラウザー自体の接続や、異なる出力ディレクトリの発行処理は並列性を維持します。
...
4. エンドポイント ファイルが非空になるまで約 1 秒間隔でポーリング待機
...
- `browser-server.js` は Linux/WSL では `chrome-wrapper.sh` を直接使用するため、`chrome-wrapper.sh` の DevTools readiness 待機とバージョン フォールバック機能が利用されます
- `browser-server.js` は Windows ネイティブ Node.js では `chrome-wrapper.sh` を使用せず、従来どおり Puppeteer の起動設定に従います
