...
本 Markdown は、Pandoc によるドキュメント発行のサンプルおよび運用方法をまとめたものである。
...
第 1 レベルのタイトルが、自動的に文書のタイトルになる。(title の指定は無視される)

発行時の日時が、文章の日時となる。(date の指定は無視される)
