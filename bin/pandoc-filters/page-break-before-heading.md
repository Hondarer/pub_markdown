...
見出し 2〜N がページ下部に配置される場合、直前に改ページを挿入する Pandoc Lua フィルター。
...
pandoc input.md -o output.docx --lua-filter=page-break-before-heading.lua \
  -M page-threshold=40 \
  -M chars-per-page=1200 \
  -M heading-level-to=4
```
...
| オプション | デフォルト | 説明 |
|-----------|-----------|------|
| `page-threshold` | 50 | 改ページを挿入する閾値 [%] |
| `chars-per-page` | 1500 | 1 ページあたりの推定文字数 |
| `heading-level-to` | 3 | 対象見出しレベルの上限 (2〜N) |
...
AST を走査しながら各ブロック要素の「文字数相当」を累積し、`chars-per-page` で割った剰余からページ内位置を推定する。見出し 2〜N が `page-threshold` [%] 以上の位置に来る場合、直前に改ページ (OOXML の `<w:br w:type="page"/>`) を挿入する。
