...
すでに `PUPPETEER_CACHE_DIR` が設定されていて、Chromium がセットアップ済みの環境では、PUPPETEER_SKIP_DOWNLOAD を指定して Puppeteer のインストールを実施する。

```text
cd {projectFolder}/bin
PUPPETEER_CACHE_DIR=1 npm install
```

Chromium がセットアップされていない環境では、以下でセットアップを行う。  
